...
extension UIViewController {
    func createRecordAlert(with title: String, _ message: String, _ actionButtonTitle: String, _ cancelButtonTitle: String, _ placeholder: String, records: [RecordEntity]?, completion: @escaping(String) -> ()) {
        ...
        alertController.addTextField { (textField) in
            ...
            NotificationCenter.default.addObserver(forName: UITextField.textDidChangeNotification, object: textField, queue: OperationQueue.main, using:
                {_ in
                    ...
                    guard records?.contains(where: { $0.title == textField.text }) == false else { actionButton.isEnabled = false; return }
